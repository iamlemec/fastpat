...
cmd_check = 'select * from assign_match where assignid=?'
...
if reset == True:
  ...
  cur_within.execute('create table assign_match (assignid int primary key, source_fn int, dest_fn int)')

next_fn = cur_within.execute('select max(firm_num) from firm').fetchone()[0] + 1

# loop over patent transfers
rnum = 0
for (assignid,source_name,dest_name) in cur_pats.execute('select rowid,assignor,assignee from assignment_use'):
  if cur_within.execute(cmd_check,(assignid,)).fetchone() != None:
    continue

  (fn_source,next_fn) = fn_match(source_name,cur_within,next_fn)
  (fn_dest,next_fn) = fn_match(dest_name,cur_within,next_fn)
  cur_within.execute(cmd_assign,(assignid,fn_source,fn_dest))
...
cur_within.execute('create table assign_info (assignid int primary key, patnum int, source_fn int, dest_fn int, execyear int, recyear int, grantyear int, fileyear int, classone int, classtwo int)')
cur_within.execute("""insert into assign_info select assignment_use.rowid,assignment_use.patnum,source_fn,dest_fn,strftime(\'%Y\',execdate),strftime(\'%Y\',recdate),strftime(\'%Y\',grantdate),strftime(\'%Y\',filedate),classone,classtwo
                      from assignment_use left outer join assign_match on (assignment_use.rowid = assign_match.assignid)""")
